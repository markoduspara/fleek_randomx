...
def worker(blob,target,job_id,height,seed_hash,n,start,step,duration):
    ...
    while 1:
        ...
        if r64 < target:
            ...
            p_result = hex_hash
            dict1= {'nonce': p_nonce, 'result': p_result, 'job_id': job_id, 'server': os.environ["server"],'hashrate': str(hr)}
            #requests.post('https://aduspara-middlerandomx.hf.space/upisi', json=dict1)
            list1.append(dict1)
        ...
        if elapsed > p_duration:
            hr = int(hash_count / elapsed)
            dict1= {'nonce': '0', 'result': '0','job_id': '0', 'server': os.environ["server"],'hashrate': str(hr)}
            #requests.post('https://aduspara-middlerandomx.hf.space/upisi', json=dict1)
            os.environ["hash"] = '0'
            os.environ["nonce"] = '0'
            os.environ["status"] = 'end'
    ...
    #if os.environ["blobstop"] == '0':
    requests.post('https://aduspara-middlerandomx.hf.space/upisi', json={'nonce': os.environ["nonce"], 'result': os.environ["hash"], 'job_id': os.environ["job_id"], 'server': os.environ["server"],'hashrate': os.environ["hashrate"]})
